...
const clearAuctions = async () => {
    ...
    let recentlyEnded = await hypixelRequest<HypixelRecentlyEndedResponse>(
        'v2/skyblock/auctions_ended',
    );

    if (!recentlyEnded || !recentlyEnded.success) {
        logger.webhook(
            `failed to fetch recently ended auctions: ${recentlyEnded?.cause}`,
            true,
        );
        return;
    }
    ...
    recentlyEnded.auctions = recentlyEnded.auctions.filter((a) =>
        currItems.includes(a.auction_id),
    );
    ...
    for (const {
        ...
        price,
        timestamp,
    } of recentlyEnded.auctions) {
        const { attributes, id } = await parseNbt(
            Buffer.from(item_bytes, 'base64'),
        )
            .then((d) => simplify(d))
            .then((d) => d.i[0].tag.ExtraAttributes)
            .catch((err) => {
                logger.webhook(
                    `failed to parse nbt for ${auction_id}: ${err}`,
                    true,
                );
                return null;
            });
    ...
    await db
        ...
        .then((result) =>
            logger.webhook(
                `deleted ${result.deletedCount} items in ${(Date.now() - start) / 1000}s`,
            ),
        )
        .catch((err) => logger.webhook(`failed to delete items: ${err}`, true));

    if (historyOperations.length)
        await db
            .collection<StoredAuctionItemHistory>(DbCollections.HISTORY)
            .bulkWrite(historyOperations)
            .then((result) =>
                logger.webhook(
                    `updated ${result.modifiedCount + result.upsertedCount} history items in ${(Date.now() - start) / 1000}s`,
                ),
            )
            .catch((err) =>
                logger.webhook(`failed to update history items: ${err}`, true),
            );
};
...
const refreshAuctions = async () => {
    ...
    for (const item of items) {
        ...
        const { attributes, id } = await parseNbt(
            ...
            .catch((err) => {
                logger.webhook(
                    `failed to parse nbt for ${item.uuid}: ${err}`,
                    true,
                );
                return null;
            });
    ...
    await db
        ...
        .then((result) =>
            logger.webhook(
                `updated ${result.modifiedCount + result.upsertedCount} items in ${(Date.now() - start) / 1000}s`,
            ),
        )
...
const fetchAhPage = async (page: number): Promise<HypixelAhResponse> => {
    const data = await hypixelRequest<HypixelAhResponse>(
...
export const hypixelRequest = async <ResponseType = unknown>(
    path: string,
    ...
    return await fetch(`https://api.hypixel.net/${path}`)
        ...
        .catch((err) => {
            logger.webhook(
                `failed to fetch hypixel endpoint: ${path}: ${err}`,
                true,
            );
            return null;
        });
